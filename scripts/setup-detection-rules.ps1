    ...
    try {
        ...
        if ($errorMessage -match "already_exists" -or $errorMessage -match "409") {
            ...
            try {
                ...
                $existingRules = Invoke-RestMethod -Uri $searchUri -Method Get
                ...
                if ($existingRule) {
                    ...
                    Write-Host "✗ No se encontró la regla existente para actualizar" -ForegroundColor Red
                }
            } catch {
...
# ============================================
Write-Host "[1/3] Creando regla de detección: SQL Injection..." -ForegroundColor Cyan
...
"@
    ...
    false_positives = @(
        "Consultas legítimas que contengan 'union' como término de búsqueda"
    )
    threat = @(
        @{
...
Create-DetectionRule -RuleName "Detección SQL Injection - Juice Shop" -RuleConfig $sqliRule | Out-Null
Write-Host ""

# ============================================
# REGLA 2: XSS (Cross-Site Scripting)
# ============================================
Write-Host "[2/3] Creando regla de detección: XSS..." -ForegroundColor Cyan
...
(url.original:*"<script*" or
 ...
 url.original:*"javascript:" or
 http.request.body.content:*"<img*" or
 message.keyword:*"<svg*" or
 threat.indicator.type: "xss")
"@
    index = @(
        "filebeat-juice-shop-*",
        "filebeat-nginx-*",
    ...
    false_positives = @(
        "Productos o reseñas legítimas que incluyan HTML permitido"
    )
    threat = @(
        @{
            framework = "MITRE ATT&CK"
...
Create-DetectionRule -RuleName "Detección Cross-Site Scripting (XSS) - Juice Shop" -RuleConfig $xssRule | Out-Null
Write-Host ""

# ============================================
# REGLA 3: Burst/Scanning
# ============================================
Write-Host "[3/3] Creando regla de detección: Burst/Scanning..." -ForegroundColor Cyan
...
"@
    ...
    references = @()
    false_positives = @(
        "Monitoreo agresivo de uptime. Documenta host de monitoreo y exclúyelo"
    )
    threat = @(
        @{
            framework = "MITRE ATT&CK"
...
Create-DetectionRule -RuleName "Detección de Scanning/Burst - Juice Shop" -RuleConfig $burstRule | Out-Null
Write-Host ""
...
try {
    ...
    $allRules = Invoke-RestMethod -Uri $rulesUri -Method Get
